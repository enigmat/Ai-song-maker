...
// Icon Components (kept as they are)
const GeneratorIcon = () => (
...
const VocalSynthIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <path d="M15.5 2.5a.5.5 0 01.5.5v1a.5.5 0 01-1 0v-1a.5.5 0 01.5-.5zM17 4.5a.5.5 0 01.5.5v1a.5.5 0 01-1 0v-1a.5.5 0 01.5-.5zM14 1a.5.5 0 01.5.5v1a.5.5 0 01-1 0v-1a.5.5 0 01.5-.5z" />
    </svg>
);
...
export const Tabs: React.FC<TabsProps> = ({ activeTool, onSelectTool, onShowRecipe }) => {
    ...
    const currentTool = useMemo(() => toolList.find(tool => tool.id === activeTool) || toolList[0], [activeTool]);
    ...
    return (
        <div className="mt-8 relative" ref={dropdownRef}>
            <button
                ...
                className="w-full flex items-center justify-between px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg shadow-md text-white font-semibold hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
                <span className="flex items-center gap-3">
                    <span className="text-purple-400">{currentTool.icon}</span>
            ...
            {isOpen && (
                <div className="absolute top-full mt-2 w-full bg-gray-800 border border-gray-700 rounded-lg shadow-2xl z-20 max-h-96 flex flex-col animate-fade-in-fast">
                    <div className="p-2 border-b border-gray-700">
                        <input
                            ...
                            className="w-full px-3 py-2 bg-gray-900 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"
                        />
                    </div>
                    <div className="overflow-y-auto">
                        {filteredTools.map(tool => (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors text-sm ${activeTool === tool.id ? 'bg-purple-600/50 text-white font-semibold' : 'text-gray-300 hover:bg-gray-700'}`}
                            >
                        ...
                        <button
                            onClick={() => { onShowRecipe(); setIsOpen(false); }}
                            className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors text-gray-300 hover:bg-gray-700 text-sm"
                        >
                           <span className="text-gray-400"><RecipeIcon /></span>
                           Recipe Mode
                        </button>
                    </div>
                </div>
